...
function restartGame() {
  ...
  window.clearInterval(ghostsInterval);
  window.clearInterval(timeInterval);
  ghostsInterval = setInterval(moveGhostSmartly, 500);
  interval = setInterval(UpdatePosition, 200);
  timeInterval = setInterval(countTimer, 100);
  Start();
}
...
function moveGhost(creature, way) {
  if (way == 1) {
    ...
    if (
      creature.y > 0 &&
      board[creature.x][creature.y - 1] != 4 &&
      !(
        (creature.y == 5 || creature.y == 6) &&
        (creature.x < 3 || creature.x > 12)
      )
    ) {
      creature.y--;
    }
  ...
  if (way == 3) {
    ...
    if (creature.x > 0 && board[creature.x - 1][creature.y] != 4) {
      creature.x--;
    } else if (creature.y == 5 && creature.x == 0) {
      //need to teleport him to the other side
      creature.x = 15;
    }
  }
  if (way == 4) {
    ...
    if (creature.x < 15 && board[creature.x + 1][creature.y] != 4) {
      creature.x++;
    } else if (creature.y == 5 && creature.x == 15) {
      //need to teleport him to the other side
      creature.x = 0;
    }
  }
...
function moveGhostSmartly() {
  for (let i = 0; i < ghosts.length; i++) {
    ...
    if (randNum == 1) {
      // Down = 2
      if (PacmanY > CordianteY) {
        moveDown(ghosts[i]);
      }
      // Up = 1
      else if (PacmanY < CordianteY) {
        moveUp(ghosts[i]);
      }
      // Right = 4
      else if (Math.abs(PacmanX - (CordianteX + 1)) < disX) {
        moveRight(ghosts[i]);
      }
      // Left = 3
      else if (Math.abs(PacmanX - (CordianteX - 1)) < disX) {
      ...
      // Right = 4
      if (Math.abs(PacmanX - (CordianteX + 1)) < disX) {
        moveRight(ghosts[i]);
      }
      // Left = 3
      else if (Math.abs(PacmanX - (CordianteX - 1)) < disX) {
